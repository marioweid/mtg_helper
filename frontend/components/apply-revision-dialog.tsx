  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div
        ...
        className="w-full max-w-lg rounded-xl border border-white/15 bg-gray-950 p-5 shadow-2xl"
      >
        <h2 id="revision-title" className="text-lg font-semibold text-white">
          Apply deck revision
        </h2>
        <p className="mt-1 text-xs text-gray-400">
          {plans.length} selected change{plans.length === 1 ? "" : "s"} · {physicalCount} →{" "}
          {physicalCount + delta} physical cards
        </p>
          ...
          {plans.map((plan) => (
            <li key={plan.id} className="flex justify-between gap-3 text-gray-300">
              <span>
                ...
                {plan.quantity} {plan.name}
              </span>
              <span className="text-xs text-gray-500">
                {plan.collection_id
                  ? (plan.owned_in.find((item) => item.id === plan.collection_id)?.name ??
                    collections.find((item) => item.id === plan.collection_id)?.name ??
                    "collection")
                  : "no collection"}
              </span>
            </li>
        ...
        <label className="mt-3 block text-xs font-medium text-gray-300">
          ...
          <textarea
            ...
            maxLength={2000}
            rows={3}
            className="mt-1 w-full resize-none rounded-lg border border-white/15 bg-black/40 px-3 py-2 text-sm text-white"
          />
        </label>
